...
export const InvestmentForm: React.FC<InvestmentFormProps> = ({
  ...
  const validateForm = () => {
    ...
    if (formData.profitSharingModel === 'PROPORTIONAL_BY_AMOUNT' || formData.profitSharingModel === 'TIERED') {
      if (!formData.profitPercentage || formData.profitPercentage <= 0 || formData.profitPercentage > 100) {
        errors.profitPercentage = 'Profit percentage must be between 0.1% and 100%'
      }
    }
  ...
  const investmentTypeOptions: { value: InvestmentType; label: string; description: string }[] = [
    {
      value: 'SHOP_WIDE',
  ...
  const profitSharingOptions: { value: ProfitSharingModel; label: string; description: string }[] = [
    {
  ...
  const getEstimatedReturn = () => {
    if (formData.amount > 0 && formData.profitPercentage) {
      const monthlyReturn = (formData.amount * formData.profitPercentage) / 100 / 12
    ...
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
              ...
              {investmentTypeOptions.map((option) => (
                <label
                  ...
                  className={`flex items-center space-x-3 p-3 border rounded-lg cursor-pointer hover:bg-gray-50 ${
                    formData.investmentType === option.value ? 'border-blue-500 bg-blue-50' : 'border-gray-200'
                  }`}
                >
                  <input
                    type="radio"
            ...
            <div className="grid grid-cols-1 gap-3">
              {profitSharingOptions.map((option) => (
                <label
                  key={option.value}
                  className={`flex items-center space-x-3 p-3 border rounded-lg cursor-pointer hover:bg-gray-50 ${
                    formData.profitSharingModel === option.value ? 'border-blue-500 bg-blue-50' : 'border-gray-200'
                  }`}
                >
                  <input
                    type="radio"
                    ...
                    onChange={(e) => handleInputChange('profitSharingModel', e.target.value as ProfitSharingModel)}
                    className="text-blue-600"
                  />
                  <div>
                    <div className="font-medium">{option.label}</div>
          ...
          {(formData.profitSharingModel === 'PROPORTIONAL_BY_AMOUNT' || formData.profitSharingModel === 'TIERED') && (
            <div className="space-y-2">
              ...
              <Input
                ...
                className={validationErrors.profitPercentage ? 'border-red-500' : ''}
              />
              {validationErrors.profitPercentage && (
                <p className="text-sm text-red-600">{validationErrors.profitPercentage}</p>
              )}
            </div>
          )}
          ...
          {formData.profitSharingModel === 'FIXED_SHARES' && (
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => handleInputChange('fixedShares', parseInt(e.target.value) || 0)}
                placeholder="Enter number of shares"
                className={validationErrors.fixedShares ? 'border-red-500' : ''}
              />
              {validationErrors.fixedShares && (
                <p className="text-sm text-red-600">{validationErrors.fixedShares}</p>
              )}
            </div>
          )}
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              <Input
                ...
                onChange={(e) => handleInputChange('maturityDate', e.target.value ? e.target.value + 'T00:00:00' : undefined)}
                className={`pl-10 ${validationErrors.maturityDate ? 'border-red-500' : ''}`}
              />
            </div>
            {validationErrors.maturityDate && (
              <p className="text-sm text-red-600">{validationErrors.maturityDate}</p>
            )}
            <p className="text-xs text-gray-500">Leave empty for indefinite investment</p>
          </div>
              ...
              <div className="flex items-start space-x-2">
                ...
                <div>
                  ...
                  <div className="text-sm text-blue-800 mt-1">
                    <p>Investment Amount: <strong>{formatCurrency(formData.amount)}</strong></p>
                    <p>Expected Monthly Return: <strong>{formatCurrency(getEstimatedReturn())}</strong></p>
                    <p>Annual Return: <strong>{formatCurrency(getEstimatedReturn() * 12)}</strong></p>
                  </div>
