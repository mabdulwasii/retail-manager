...
export const Select: React.FC<SelectProps> = ({
  ...
  // Use refs to access current values without triggering callback recreations
  ...
  useEffect(() => {
    ...
    // Only update if position actually changes to prevent unnecessary re-renders
    setDropdownPosition((prev) => (prev === newPosition ? prev : newPosition));
  }, [open]);
  ...
  // Stable callbacks that don't recreate on open/value changes
  const registerItem = useCallback(
    (itemValue: string, label: React.ReactNode) => {
      const isNewItem = !itemsRef.current.has(itemValue);
      itemsRef.current.set(itemValue, label);

      // Only trigger re-render if this is the currently selected value
      // This ensures SelectValue displays correctly on mount
      if (isNewItem && valueRef.current === itemValue) {
  ...
  return (
    <div
      ...
      className={cn(
        ...
        context.dropdownPosition === "bottom"
          ? "top-full mt-1"
          : "bottom-full mb-1",
